		...
		$col = array('car', 'pat', 'mat', 'nom', 'nom2', 'apes', 'eciv', 'sex', 'nac', 'ing', 'mes', 'a_o', 'uni', 'desg', 
     	...
     	Excel::selectSheetsByIndex(0)->filter('chunk')->select($col)->load($filename,$reader)->chunk(500, function($results) {
			...
			foreach ($results as $result) {
				...
				if(!$result->car)
				{
					$message = "Carnet Vacío Solo se importaron " . $countAfi . " Afiliados y " . $countApor.  " Registros de Planilla" ;
					Session::flash('message', $message);
					return view('import.import_select');
					break;
				}
				...
				if ($afiliado === null) {
		            ...
		            $rules = [
		                'car' => 'regex:/^[a-záéíóúàèìòùäëïöüñ\s]+$/i',
		            ];
		            $messages = [
            			'ape.required' => 'El campo apellidos tiene que ser solo letras',
        			];
